...
diseases = [
    ...
    'Pneumonia', 'Kidney Disease', 'Gastroenteritis', 'Depression', 'Lung Cancer', # Added Lung Cancer
    'No Disease'
]
all_columns = [
    ...
    'smoking_status', # Added smoking_status column
...
for _ in range(num_entries):
    ...
    row['smoking_status'] = np.random.choice(['Non-Smoker', 'Smoker'], p=[0.7, 0.3]) # General population smoking status
    for col in row:
        if 'Score' in col or 'Level' in col or 'Severity' in col: row[col] = np.random.randint(0, 3)

    # Adjusted probability for 10 diseases + 'No Disease'
    assigned_disease = np.random.choice(diseases, p=[0.08]*10 + [0.20])
    ...
    # --- NEW DISEASE PROFILE ---
    elif assigned_disease == 'Lung Cancer':
        row['Age'] = np.random.randint(55, 90)
        # Heavily bias this condition towards smokers
        if np.random.rand() < 0.90: # 90% of lung cancer cases are smokers
            row['smoking_status'] = 'Smoker'
        row['Cough_Severity'] = np.random.randint(6, 11)
        row['Fatigue_Level'] = np.random.randint(7, 11)
        row['Shortness_of_Breath'] = 1
        row['Chest_Pain_Type'] = np.random.choice([0, 1, 2], p=[0.2, 0.4, 0.4])
    ...
    if chance_of_mislabel < 0.07: # 7% chance to mislabel
        if assigned_disease == 'Influenza':
            assigned_disease = 'Pneumonia'
        elif assigned_disease == 'Hypertension':
            assigned_disease = 'Kidney Disease'
        elif assigned_disease == 'Diabetes':
            assigned_disease = 'Heart Disease'
        elif assigned_disease == 'Pneumonia':
...
df = df.fillna(0)
# Conversion logic safely ignores object types like 'Gender' and 'smoking_status'
